...
void Network::train(double learning_rate) {
    ...
    for (int ep = 0; ep < epochs; ep += batch_size) {
        ...
        for (int b = 0; b < batch_size; b++) {
            ...
            feedforward(&a);
            y = transformOutput(y_int);
            // cerr << "epoch "<<ep + b<<" with error "<< getError(a,y_int)<<" \n";
        ...
        cerr << "epoch "<<ep <<" with ratio "<< (1 - (double)pososto/(double)batch_size)*100<<" \n";
...
void Network::backpropagate(double* d_L) {
    ...
    for (int i = numOfLayers - 2; i >= 0; i--) {
        ...
        w_d = matrix_vector_mull(sizeOfLayers[i + 1], sizeOfLayers[i + 2], w[i + 1], delta[i + 1]);
